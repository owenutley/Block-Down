...
const colorToBlockType = (color: string) => {
...
const convertPuzzleToLevelConfig = (puzzle: any) => {
  return {
...
export const Splash = () => {
  ...
  useEffect(() => {
    const fetchSplash = async () => {
      try {
        const splashPuzzles = await trpc.puzzle.getByDifficulty.query('splash');
        if (splashPuzzles && splashPuzzles.length > 0) {
          const config = convertPuzzleToLevelConfig(splashPuzzles[0]);
