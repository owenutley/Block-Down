  ...
  return (
    <div className="flex flex-col gap-4 text-left">
      <div className="flex items-center justify-between gap-4 border-b border-gray-700/60 pb-3">
        ...
        <span className="bg-gray-900 border border-gray-700 text-gray-400 text-[10px] px-2 py-0.5 rounded font-mono select-all">
          {puzzle.difficulty}
        </span>
      </div>
...
const ALL_SHAPES = ALL_SHAPE_IDS;
...
const ThemeCustomizerPanel = ({
  ...
  const [selectedTheme, setSelectedTheme] = useState<ThemeId>('neon');
  const [localConfigs, setLocalConfigs] = useState<Record<ThemeId, ThemeConfig>>(themeConfigs);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const configToSave = localConfigs[selectedTheme] || DEFAULT_THEME_CONFIGS[getBaseThemeId(selectedTheme)];
      await trpc.theme.updateConfig.mutate({
        themeId: selectedTheme,
        config: configToSave,
      });
      await onSaveThemeConfigs();
      showToast({ text: `Successfully updated ${getThemeName(selectedTheme)} theme config!`, appearance: 'success' });
    } catch (err) {
      console.error(err);
      showToast({ text: 'Failed to save theme config', appearance: 'neutral' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleReset = async () => {
    setResetting(true);
    try {
      await trpc.theme.resetConfig.mutate({ themeId: selectedTheme });
      await onSaveThemeConfigs();
      showToast({ text: `Successfully reset ${getThemeName(selectedTheme)} config!`, appearance: 'success' });
    } catch (err) {
      console.error(err);
      showToast({ text: 'Failed to reset theme config', appearance: 'neutral' });
    } finally {
      setResetting(false);
    }
  };
...
export function DevPanel({
  ...
  const [themeConfigsState, setThemeConfigsState] = useState<Record<ThemeId, ThemeConfig>>(
    themeConfigs || DEFAULT_THEME_CONFIGS
  );
  const [themesState, setThemesState] = useState<Theme[]>(themes || []);

  const fetchThemeConfigs = async () => {
    try {
      const configs = await trpc.theme.getAllConfigs.query();
      setThemeConfigsState(configs);
    } catch (e) {
      console.error('Failed to fetch theme configs:', e);
    }
  };
  ...
  useEffect(() => {
    if (themeConfigs) {
      setThemeConfigsState(themeConfigs);
    } else {
      void fetchThemeConfigs();
    }
  }, [themeConfigs]);
  ...
  const handleSaveThemeConfigs = async () => {
    if (onSaveThemeConfigs) {
      await onSaveThemeConfigs();
    } else {
      await fetchThemeConfigs();
      await fetchThemes();
    }
  };
            ...
            <div className="bg-gray-800 rounded-lg p-6 sticky top-6 border border-gray-700 shadow-xl">
              ...
              <div className="bg-gray-900/80 border border-gray-700 rounded-lg p-4 mb-6 text-left">
                ...
                <div className="space-y-3">
                  ...
                  {mappingPostId !== null && (
                    <div className="bg-black/35 border border-gray-700 rounded p-2 text-xs text-gray-400 space-y-1 font-mono">
                          ...
                          <div className="flex gap-4">
                            <div className="flex-1">
                              ...
                              <div className="flex border border-gray-700 rounded overflow-hidden">
                                <button type="button" onClick={() => setGridWidth(w => Math.max(3, w - 1))} className="flex-1 bg-gray-700 hover:bg-gray-600 font-bold py-1 text-xs">-</button>
                                <button type="button" onClick={() => setGridWidth(w => Math.min(15, w + 1))} className="flex-1 bg-gray-700 hover:bg-gray-600 font-bold py-1 text-xs">+</button>
                              </div>
                            </div>
                            <div className="flex-1">
                              <span className="text-[10px] text-gray-400 block mb-1">Height: {gridHeight}</span>
                              <div className="flex border border-gray-700 rounded overflow-hidden">
                                <button type="button" onClick={() => setGridHeight(h => Math.max(3, h - 1))} className="flex-1 bg-gray-700 hover:bg-gray-600 font-bold py-1 text-xs">-</button>
                                <button type="button" onClick={() => setGridHeight(h => Math.min(15, h + 1))} className="flex-1 bg-gray-700 hover:bg-gray-600 font-bold py-1 text-xs">+</button>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div key={puzzle.id} className="flex flex-col">
                          ...
                          <div
                            ...
                            className={cn(
                              ...
                              isSelected ? "border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.3)] bg-gray-800" : "border-gray-700/60"
                            )}
                          >
